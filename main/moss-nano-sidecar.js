...
function createMossNanoSidecarAdapter(options = {}) {
  ...
  let buffer = "";
  let lastConfig = null;
  let lastStatus = unavailableStatus(null, "sidecar-not-started", "MOSS Nano sidecar has not been started.");
  ...
  function handleMessage(message) {
    ...
    if (message.type === "ready") {
      lastStatus = message.ready === false || message.ok === false
        ? unavailableStatus(lastConfig, message.reason ?? "sidecar-not-ready", message.detail ?? "MOSS Nano sidecar is not ready.", {
    ...
    if (message.type === "result" || message.type === "error") {
      const result = message.type === "error"
        ? failureResponse(message.reason ?? "sidecar-request-failed", message.detail ?? message.error ?? "MOSS Nano request failed.", {
            requestId: message.requestId ?? null,
            ownerToken: message.ownerToken ?? null,
          })
      ...
      if (message.type === "result" && !mockMode() && result?.syntheticAudio !== false) {
        settlePending(`request:${message.requestId}`, failureResponse(
          ...
          "MOSS Nano real sidecar mode rejected synthetic or unclassified audio output.",
          {
            requestId: message.requestId ?? null,
            ownerToken: message.ownerToken ?? null,
          },
  ...
  function handleStdout(chunk) {
    ...
    for (const line of lines) {
      ...
      try {
        ...
        lastStatus = unavailableStatus(lastConfig, "sidecar-protocol-error", `MOSS Nano sidecar emitted invalid JSON: ${error.message}`);
      }
    }
  }
  ...
  function startProcess(config) {
    ...
    lastStatus = {
      ...
      status: "loading",
      reason: null,
      detail: "MOSS Nano sidecar is starting.",
      ready: false,
      loading: true,
      recoverable: true,
      config,
    ...
    child.stderr?.on("data", (chunk) => {
      ...
      if (text) {
        lastStatus = { ...lastStatus, detail: text.slice(-500) };
      }
    });
    child.on("error", (error) => {
      const status = unavailableStatus(lastConfig, error.code || "sidecar-spawn-failed", error.message || "MOSS Nano sidecar failed to spawn.");
      lastStatus = status;
      if (startDeferred) {
        const deferred = startDeferred;
        startDeferred = null;
    ...
    child.on("exit", (code, signal) => {
      const status = shuttingDown
        ? unavailableStatus(lastConfig, "sidecar-shutdown", "MOSS Nano sidecar was shut down.", { status: "shutdown", recoverable: true })
        : unavailableStatus(lastConfig, "sidecar-exited", `MOSS Nano sidecar exited with code ${code ?? "unknown"}${signal ? ` and signal ${signal}` : ""}.`);
  ...
  function terminateChild() {
    ...
    try {
      ...
      // Ignore stream cleanup errors while tearing down a hung runtime.
    }
    if (process.platform === "win32" && target.pid) {
      try {
        ...
        // Fall back to normal child termination below.
      }
    }
    target.kill?.();
  }
  ...
  function queueRequest(key, payload, timeoutMs) {
    ...
    const timer = setTimeout(() => {
      pending.delete(key);
      deferred.resolve(failureResponse("sidecar-timeout", "MOSS Nano sidecar did not respond before the timeout.", {
  ...
  return {
    async start(config) {
      ...
      startTimer = setTimeout(() => {
        ...
        const deferred = startDeferred;
        startDeferred = null;
        lastStatus = unavailableStatus(lastConfig, "sidecar-start-timeout", "MOSS Nano sidecar did not report ready before the startup timeout.");
        terminateChild();
        deferred.resolve(lastStatus);
      }, commandTimeoutMs());
    ...
    async request(command, payload) {
      if (!child || !lastStatus.ready) {
        return failureResponse("sidecar-not-ready", "MOSS Nano sidecar is not ready.", {
          requestId: payload?.requestId ?? null,
          ownerToken: payload?.ownerToken ?? null,
        });
      }
      ...
      if (!writeCommand({ command, ...payload })) {
        settlePending(key, failureResponse("sidecar-not-ready", "MOSS Nano sidecar is not ready.", {
          requestId: payload?.requestId ?? null,
          ownerToken: payload?.ownerToken ?? null,
        }));
      }
    ...
    async shutdown() {
      ...
      rejectAll("sidecar-shutdown", "MOSS Nano sidecar was shut down.");
      lastStatus = unavailableStatus(lastConfig, "sidecar-shutdown", "MOSS Nano sidecar was shut down.", {
        ok: true,
        status: "shutdown",
        ready: false,
      });

...
export function validateNarrationImport(
  ...
  if (conflictingNames.length > 0) {
    warnings.push(`${conflictingNames.length} profile(s) share names with existing profiles and will be added as duplicates alongside the current Kokoro defaults`);
  }

...
## Current System State (v1.76.0 — queue RED depth 2, Reading Experience v2, 1 open bug)
...
- **FLOW-ZONE-AUTO complete** — Descending auto-advancing reading zone replaces fixed-position scroll. Zone starts at 15% from viewport top (`FLOW_ZONE_INITIAL_TOP`), walks downward as words advance (text stays still, mask moves), instant page-jump-scroll when zone bottom crosses 67% (`FLOW_ZONE_RESET_THRESHOLD`), resets to top. Zone position dropdown removed from ReaderBottomBar; Lines slider retained. `FlowScrollEngine.advanceZone()` replaces `scrollToLine()`. `getFlowZoneHost()` added to `FoliateViewAPI`. `onZoneTopChange` callback drives CSS custom properties. Pre-existing render loop fixed: `setMediaSessionBook` wrapped in `useCallback` to stabilize identity. 14 new tests (`tests/flowZoneAuto.test.ts`) + 1 regression test (`tests/useNarrationMount.test.tsx`). v1.76.0.
- **Queue pointer** — `docs/governance/sprint-queue.xlsx` is the authoritative queue source. Queue depth is RED at 2 (0 full specs + 2 stubs): UX-POLISH-1 → TTS-QUAL-CI-1. /roadmap-review required before next dispatch.
...
- Active queue: RED depth 2 (0 full specs, 2 stubs) in `docs/governance/sprint-queue.xlsx`. Finish line: **TTS Quality Confidence + Reading Experience v2** — narration UX polish + quality regression gates. Remaining: UX-POLISH-1 → TTS-QUAL-CI-1. TTS Architecture Complete phase landed. Engine posture: Kokoro is the sole active engine; MOSS-Nano and Pocket TTS are dormant/disabled; Qwen is retired/disabled. Desktop v2.0 shipped. KOKORO-EXPORT-1 remains deferred.
...
- 2,668 tests across 194 test files

...
- **Queue pointer** — `docs/governance/sprint-queue.xlsx` is the authoritative queue source. Next approved sprint is TTS-PARITY-1, dispatch-ready on `main`. Queue depth is 6 (4 full specs + 2 stubs): TTS-PARITY-1 → NARR-SPOKEN-1 → NARR-CURSOR-2 → TTS-EVAL-3 → UX-POLISH-1 → TTS-QUAL-CI-1. NARR-PAUSE-1 is complete and landed.

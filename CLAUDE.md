...
- **Queue pointer** — `docs/governance/sprint-queue.xlsx` is the authoritative queue source. Next approved sprint is TTS-EVAL-3, dispatch-ready on `main`. Queue depth is 3 (1 full spec + 2 stubs): TTS-EVAL-3 → UX-POLISH-1 → TTS-QUAL-CI-1. NARR-CURSOR-2 is complete and landed.

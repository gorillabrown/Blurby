...
## Current System State (v1.75.1 — queue GREEN depth 5 [3 full specs + 2 stubs], 2 open bugs)
...
- **Queue:** GREEN depth 5 (3 full specs at positions 1-3: SINGLE-INSTANCE-LOCK-1, THEME-SYNC-1, NARRATE-CLOSED-LOOP-CURSOR; 2 stubs at positions 4-5: UX-POLISH-1, HYG-XLSX-DASHBOARD-RESTORE) in `docs/governance/sprint-queue.xlsx`. Next: SINGLE-INSTANCE-LOCK-1 — Electron single-instance lock. SINGLE-INSTANCE-LOCK-1 + THEME-SYNC-1 are hotfix sprints (F1, BUG-182). NARRATE-CLOSED-LOOP-CURSOR is the sole shared-core sprint and must run alone.
- **Open bugs:** 2 — BUG-154 (parked, likely not a bug, needs live verification), BUG-182 (Settings panel doesn't track light-dark theme toggle; reported 2026-05-28).
...
- **Most recent sprint:** EXT-PAIR-1 — Chrome extension pairing auth timeout repair (2026-05-29).

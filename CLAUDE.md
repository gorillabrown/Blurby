...
- **Queue pointer** — `docs/governance/sprint-queue.xlsx` is the authoritative queue source. Next approved sprint is NARR-PAUSE-1, dispatch-ready on `main`. Queue depth is 7 (5 full specs, 2 stubs): NARR-PAUSE-1 → TTS-PARITY-1 → NARR-SPOKEN-1 → NARR-CURSOR-2 → TTS-EVAL-3 → UX-POLISH-1 → TTS-QUAL-CI-1. NARR-MEDIA-1 is complete and landed.

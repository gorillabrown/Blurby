...
## Current System State (Session 1 — Post-Review Baseline)
...
- **Main process** (`main.js`, 93KB): Electron shell, IPC handlers, file I/O, folder watching, data persistence, URL article extraction, format parsers
- **Preload** (`preload.js`, 4KB): Context bridge → `window.electronAPI`
- **Renderer** (`src/`): React 19 SPA
  - `App.tsx` (20KB) — Central orchestrator
  - `src/components/` — 23 UI components + 7 settings sub-pages
...
- **Tests** (`tests/`): 9 test files (135+ tests)
...
- **PR #1 not merged** — 102 commits sitting on dev branch
- **Performance optimization** — SPRINT-PERF.md planned but not executed
- **Auto-updater wiring** — electron-updater installed but not configured
- **Code signing** — documented but not obtained
- **Chrome extension** (Phase 9) — design only
- **Android app** (Phase 10) — design only
- **CI/CD** — no GitHub Actions, no automated tests on PR
- **Reading statistics** — no history.json, no stats panel
- **Drag-and-drop** — DropZone component exists but may need polish
- **Recent folders** — RecentFolders component exists but integration unclear
- **Reader exit confirmation** — not implemented
- **Symlink protection** — not implemented
- **requestAnimationFrame playback** — not migrated from setInterval

---

## Dependency Chain

PR #1 Squash-Merge → **Sprint 2** (React perf) ∥ **Sprint 3** (main.js modularization) → **Sprint 4** (main process perf) → **Sprint 5** (reader mode perf) → **Sprint 6** (polish) → **Sprint 7** (stats) ∥ **Sprint 8** (distribution) → **Phase 9** (Chrome extension) → **Phase 10** (Android)

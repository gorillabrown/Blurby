...
## Current System State (v1.75.1 — queue RED/backfill required, Desktop v2.0 conveyor active, 1 open bug)
...
- **TTS-SYNC-1 complete** — `src/utils/timingMetadataStore.ts` and `src/utils/highlightSyncController.ts` centralize narration highlight sync policy. Trusted word-native timing can drive word-synced decisions; heuristic/missing timing downgrades to chunk/segment decisions with no invented active word. Scheduler/Kokoro/useNarration publish and store timing metadata, and `ReaderContainer` consumes controller decisions without changing Flow's WPM clock, Narrate's spoken-timing clock, autoplay behavior, Qwen/provider defaults, or `lastConfirmedAudioWordRef` ownership. Verification passed: focused sync baseline 8 files / 85 tests, focused sync regression 9 files / 93 tests, full `npm test` 181 files / 2598 tests, `npm run typecheck`, `npm run build`, and `git diff --check`.
- **Queue pointer** — Next approved sprint is TTS-DIAG-1. Queue depth is 1 prepared pointer after TTS-SYNC-1 closeout; this is a RED stop signal and backfill is required before further code-changing dispatch.
...
- Active queue: RED on depth, GREEN on TTS posture — TTS-DIAG-1 is next, but queue backfill is required before further code-changing dispatch. Desktop v2.0 conveyor active; MOSS-NANO-13a–13e, POCKET-TTS-1, POLISH-1, RELEASE-1, POSTV2 remediation, and the Kokoro architecture arc through TTS-SYNC-1 are complete. KOKORO-RETIRE remains deferred even though MOSS-NANO records `NANO_RECOMMENDED_OPT_IN`.

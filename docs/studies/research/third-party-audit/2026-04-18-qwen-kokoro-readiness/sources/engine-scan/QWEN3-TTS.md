...
## Snapshot
- Track: context
- Current verdict: active candidate; empirical run requires a workstation-grade CUDA lane
...
- Open concerns: if Dispatch B runs on a host without an appropriate CUDA workstation, classify Qwen as host-blocked rather than excluded-by-policy so the final recommendation does not overstate Kokoro's proof status.

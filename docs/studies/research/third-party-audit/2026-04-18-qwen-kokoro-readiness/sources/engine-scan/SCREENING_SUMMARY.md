...
| Qwen3-TTS | context | pass | pass | workstation-first; official examples are CUDA + FlashAttention oriented | active if suitable CUDA lane is available |
...
- Keep Qwen3-TTS in the empirical lane whenever a suitable CUDA workstation is available; if the current host cannot satisfy that runtime, treat Qwen as host-blocked rather than excluded-by-policy.

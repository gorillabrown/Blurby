...
- READER-ISO-1E — Narrate Adapter + Audio Truth-Sync Ownership
...
#### READER-ISO-1E — Narrate Adapter + Audio Truth-Sync Ownership *(archived)*
...
- **Why:** Narrate is hook-owned today but the ownership is implicit — no first-class adapter boundary, so orchestration behavior leaks. Isolating Narrate is the prerequisite for `NARRATE-CLOSED-LOOP-CURSOR`: scheduler surgery on the heard-position loop is only safe once the adapter owns truth-sync.
...
- **Done when:**
  1. `NarrateModeAdapter` exists and implements the 1A contract.
  ...
  6. Behavior-preserving — no cursor/boundary math change.
  7. `npm test` passes, `npx tsc --noEmit` clean, `npm run build` succeeds, `git diff --check` clean.
- **Effort:** L. Cross-cutting runtime migration with shared-core freeze set.
- **Edit sites:** `src/reader/modes/NarrateModeAdapter.ts` (new), `tests/narrateModeAdapter.test.ts` (new).
- **Result:** Complete. `6aa47ba` added `NarrateModeAdapter` (187 lines) implementing `ReaderModeAdapter` with audio truth-sync ownership (`clockOwner: "audio-truth"`), browse-away state tracking, surface command emission, and `NarrationBridge` interface. 44 adapter tests with SRL-073 transition/no-op coverage; `c3d8776` merged to `main`. Per SRL-074, ref-heavy truth-sync RAF callbacks stay in `useReaderMode` as building blocks until integration sprint wires adapters into the hook tree. Orchestrator wiring deferred — consistent with 1C/1D pattern.

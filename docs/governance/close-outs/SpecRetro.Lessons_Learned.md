...
**Status:** **Promotion candidate** on first occurrence. Cost asymmetry (cheap to grep, expensive when a wrong count anchors an Aristotle enumeration scope or a DIAG instrumentation pass) suggests promotion shouldn't wait for a second incident — but per Standing Rules promotion convention, this entry sits as forward-defense until a second case validates the pattern.
...
**Status:** **Promote SRL-086 + SRL-087 jointly to a Standing Rule** under the umbrella formulation above. The two occurrences across two different verification axes within ~1 hour is decisive evidence the principle deserves promotion; the cost asymmetry (cheap to verify, expensive when wrong-state anchors downstream artifacts) reinforces the urgency. Recommend Standing Rule wording: *"Verify state — codebase facts (grep), environment (branch/cwd/build), and binary embodiment (rebuild) — explicitly before consequential edits, gate verdicts, or published claims. Accumulated context is not verified state; trust must be earned by an explicit verification step at the moment of consequence."*

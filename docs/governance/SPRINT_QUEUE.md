...
```
SPRINT QUEUE STATUS:
Queue depth: 0 for MOSS app-integration; 0 for queued Nano app work — APP INTEGRATION PAUSED
Next queue item: none for MOSS app integration; next Nano work, if any, should be resident runtime tuning/soak/perf only
Health: ITERATE_NANO_RESIDENT_RUNTIME — MOSS-NANO-3 is complete and records true resident reuse plus internal first decoded audio, but short RTF `1.7005` misses the `<=1.5` promotion threshold and memory growth needs soak/tuning. Do not dispatch MOSS-3, MOSS-4, or later app-integration/productization sprints unless `docs/testing/MOSS_DECISION_LOG.md` records `PROMOTE_TO_MOSS_3_CANDIDATE`, `PROMOTE_NANO_TO_APP_PROTOTYPE_CANDIDATE`, or another explicit app-integration promotion decision. Kokoro production behavior is unchanged.
```
...
| 11 | MOSS-3 | v1.79.0 | sprint/moss-3-sidecar-contract | Full | PAUSED | Blocked until the decision log records `PROMOTE_TO_MOSS_3_CANDIDATE`, `PROMOTE_NANO_TO_APP_PROTOTYPE_CANDIDATE`, or another explicit app-integration promotion decision; MOSS-NANO-3 instead recorded `ITERATE_NANO_RESIDENT_RUNTIME`. Do not dispatch sidecar, IPC, status contract, or app integration work. |
| 12 | MOSS-4 | v1.80.0 | sprint/moss-4-live-narration-strategy | Full | PAUSED | Blocked by MOSS-3 and current `ITERATE_NANO_RESIDENT_RUNTIME` state; do not add renderer narration strategy or selectable app engine path work. |
| 13 | MOSS-5 | v1.81.0 | sprint/moss-5-timing-truth | Full | PAUSED | Blocked by current `ITERATE_NANO_RESIDENT_RUNTIME` state; no MOSS timing-truth integration is queued. |
| 14 | MOSS-6 | v1.82.0 | sprint/moss-6-cache-continuity | Full | PAUSED | Blocked by current `ITERATE_NANO_RESIDENT_RUNTIME` state; no cache/prewarm/continuity work is queued. |
| 15 | MOSS-7 | v1.83.0 | sprint/moss-7-productization-gate | Full | PAUSED | Blocked by current `ITERATE_NANO_RESIDENT_RUNTIME` state; no productization gate is queued. |

**Dispatch status:** MOSS app-integration remains PAUSED. `MOSS-NANO-3` is CLOSED as `ITERATE_NANO_RESIDENT_RUNTIME`; next Nano work, if any, should stay limited to resident runtime tuning/soak/perf and must not add app integration, sidecar IPC, renderer integration, selectable engine behavior, cache/continuity work, timing-truth UI integration, Kokoro behavior changes, or Kokoro retirement work. `GOALS-6B` remains parked and independent. `KOKORO-RETIRE-1` and `KOKORO-RETIRE-2` remain paused; do not re-queue them until a separate successor lane proves continuous live playback and a separate Kokoro-retirement lane is explicitly approved.
...
| MOSS-NANO follow-up | **Paused under `ITERATE_NANO_RESIDENT_RUNTIME`.** MOSS-NANO-3 proved true resident reuse and internal first decoded audio, but did not promote because short RTF `1.7005` missed the `<=1.5` threshold and memory growth needs soak/tuning. Future Nano work, if any, should be resident runtime tuning/soak/perf only; it must not add app integration or change Kokoro behavior without an explicit promotion decision. |

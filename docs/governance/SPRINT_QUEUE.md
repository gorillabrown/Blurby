...
Status: Conditional; dispatch only after `docs/testing/MOSS_DECISION_LOG.md` records `PROMOTE_NANO_TO_APP_PROTOTYPE_CANDIDATE` or stricter after the MOSS-NANO-6C non-promotion.
...
| 16 | MOSS-NANO-7 | v1.79.0 | sprint/moss-nano-7-sidecar-ipc-prototype | Prototype sidecar | CONDITIONAL | Dispatch only after `docs/testing/MOSS_DECISION_LOG.md` records `PROMOTE_NANO_TO_APP_PROTOTYPE_CANDIDATE` or stricter after the MOSS-NANO-6C non-promotion. Adds managed main-process sidecar and IPC contract; no renderer selection or strategy switch. |
...
| MOSS-NANO follow-up | **Runtime iteration only; app integration still gated.** MOSS-NANO-6C closed as `ITERATE_NANO_RESIDENT_RUNTIME`, not app prototype promotion. Next Nano work must harden resident runtime/package evidence, lifecycle shutdown, memory slope, and adjacent RTF gates, and must not dispatch MOSS-NANO-7, app integration, renderer integration, sidecar IPC, selectable engine/cache work, Kokoro behavior change, or MOSS-3 reopen. |

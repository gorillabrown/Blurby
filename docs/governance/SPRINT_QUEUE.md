...
```
SPRINT QUEUE STATUS:
Queue depth: 6 for the gated Nano onboarding roadmap; 1 dispatch-ready runtime-only soak/package sprint; app integration still gated.
Next queue item: MOSS-NANO-6 is dispatch-ready as resident soak + packaging readiness after MOSS-NANO-5C earned a runtime-only segment-first soak-candidate gate.
Health: YELLOW/RUNTIME-SOAK — MOSS-NANO-5C closed `PROMOTE_NANO_TO_SOAK_CANDIDATE_WITH_SEGMENT_FIRST_GATE`, not app promotion. `MOSS-NANO-6` may dispatch for resident soak + packaging readiness only. `MOSS-NANO-7` through `MOSS-NANO-11` define the full Nano onboarding path but must not dispatch until `docs/testing/MOSS_DECISION_LOG.md` records `PROMOTE_NANO_TO_APP_PROTOTYPE_CANDIDATE` or stricter. Kokoro production behavior remains unchanged and Kokoro remains the operational floor.
```
...
NEXT: MOSS-NANO-6 may dispatch as resident soak + packaging readiness because 5C earned a runtime-only soak-candidate gate. Do not dispatch app integration, renderer integration, sidecar IPC, selectable engine/cache work, Kokoro behavior changes, MOSS-3 reopen, or Kokoro retirement from this closeout.
```

```text
Sprint: MOSS-NANO-6 — Resident Soak + Packaging Readiness
Status: Dispatch-ready after MOSS-NANO-5C recorded PROMOTE_NANO_TO_SOAK_CANDIDATE_WITH_SEGMENT_FIRST_GATE. Runtime soak/package only; app integration remains gated.
...
| 14 | MOSS-NANO-6 | v1.78.10 | sprint/moss-nano-6-soak-packaging-readiness | Runtime/package readiness | READY | Dispatch-ready after MOSS-NANO-5C's runtime-only segment-first soak-candidate gate. No app integration is unlocked. |
...
| MOSS-NANO follow-up | **Runtime soak/package ready; app integration still gated.** MOSS-NANO-5C promoted Nano to `PROMOTE_NANO_TO_SOAK_CANDIDATE_WITH_SEGMENT_FIRST_GATE` on segment-first evidence only. MOSS-NANO-6 may dispatch for resident soak + packaging readiness; no app integration, renderer integration, sidecar IPC, selectable engine/cache work, Kokoro behavior change, or MOSS-3 reopen is unlocked. |

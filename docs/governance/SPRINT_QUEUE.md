...
```
SPRINT QUEUE STATUS:
Queue depth: 0 for MOSS app-integration; 0 for queued Nano runtime follow-up — APP INTEGRATION PAUSED
Next queue item: none for MOSS app integration; none for Nano runtime work unless a new explicit promotion/reopen decision is recorded
Health: KEEP KOKORO ONLY — MOSS-NANO-2 records `KEEP_KOKORO_ONLY`. Do not dispatch MOSS-3, MOSS-4, or later app-integration/productization sprints unless `docs/testing/MOSS_DECISION_LOG.md` records `PROMOTE_TO_MOSS_3_CANDIDATE`, `PROMOTE_NANO_TO_APP_PROTOTYPE`, or another explicit app-integration promotion decision. Kokoro production behavior is unchanged. Nano generated local CPU audio and remains better than flagship, but current Nano evidence is not viable for live app prototype.
```
...
| 10 | MOSS-3 | v1.79.0 | sprint/moss-3-sidecar-contract | Full | PAUSED | Blocked until the decision log records `PROMOTE_TO_MOSS_3_CANDIDATE`, `PROMOTE_NANO_TO_APP_PROTOTYPE`, or another explicit app-integration promotion decision; MOSS-NANO-2 instead recorded `KEEP_KOKORO_ONLY`. Do not dispatch sidecar, IPC, status contract, or app integration work. |
| 11 | MOSS-4 | v1.80.0 | sprint/moss-4-live-narration-strategy | Full | PAUSED | Blocked by MOSS-3 and current `KEEP_KOKORO_ONLY` state; do not add renderer narration strategy or selectable app engine path work. |
| 12 | MOSS-5 | v1.81.0 | sprint/moss-5-timing-truth | Full | PAUSED | Blocked by current `KEEP_KOKORO_ONLY` state; no MOSS timing-truth integration is queued. |
| 13 | MOSS-6 | v1.82.0 | sprint/moss-6-cache-continuity | Full | PAUSED | Blocked by current `KEEP_KOKORO_ONLY` state; no cache/prewarm/continuity work is queued. |
| 14 | MOSS-7 | v1.83.0 | sprint/moss-7-productization-gate | Full | PAUSED | Blocked by current `KEEP_KOKORO_ONLY` state; no productization gate is queued. |

**Dispatch status:** MOSS app-integration remains PAUSED. `MOSS-NANO-2` is closed as `KEEP_KOKORO_ONLY`: Nano generated local CPU audio and is operationally stronger than flagship, but corrected real-text timings plus segmentation, ORT, and prewarm/cache attempts still do not justify a live app prototype. `GOALS-6B` remains parked and independent. `KOKORO-RETIRE-1` and `KOKORO-RETIRE-2` remain paused; do not re-queue them until a separate successor lane proves continuous live playback and a separate Kokoro-retirement lane is explicitly approved. Do not begin MOSS app integration or change Kokoro behavior.
...
| MOSS-NANO follow-up | **Paused under `KEEP_KOKORO_ONLY`.** MOSS-NANO-2 closed without app promotion. Future Nano work should reopen only with in-process runtime instrumentation, true session reuse, trustworthy internal first-decoded timing, and applied ORT/session options; it must not add app integration or change Kokoro behavior without an explicit promotion decision. |

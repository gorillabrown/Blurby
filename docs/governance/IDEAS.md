...
| H7 | MediaSession Integration | Register Electron MediaSession API for lock-screen / Bluetooth headphone media controls (play/pause/skip). Readest implements sentence/paragraph/chapter granularity selector via Tauri native bridge; Electron has equivalent API. Low effort, high UX value for narration. (Source: Blurby.Research findings — readest pattern.) | High | Small |
| H8 | Named-Pause State Machine | Distinguish *why* playback paused: `stop-paused`, `rate-change-paused`, `voice-change-paused`, `forward-paused`, `backward-paused`. Enables smarter resume behavior (resume at current position after rate change vs. paragraph start after manual stop). Readest implements 7-state machine. (Source: Blurby.Research findings — readest `TTSController` pattern.) | Medium | Small |
| H9 | Two-Engine Quality Swap | Start Web Speech immediately for sub-second TTFB; render Kokoro in parallel; swap on sentence boundary. Best perceived latency pattern found across all reviewed codebases. Only relevant if/when a second engine is reactivated. (Source: Blurby.Research findings — sioyek pattern.) | High | Large |
| H10 | Pronunciation Override UI | Data layer exists (`pronunciationOverrides.ts`, `overrideHash` in cache key) but no settings surface, no persistent per-book store. Port abogen's `pronunciation_store.py` pattern: per-book word→replacement map with settings UI. (Source: Blurby.Research findings — abogen pattern.) | Medium | Medium |
| H11 | EPUB3 Media Overlay SMIL Export | Export narration as EPUB3 Media Overlay — write SMIL timing metadata attaching audio to EPUB per the EPUB3 spec. Standards-track audiobook export. Fold into KOKORO-EXPORT-1 prerequisites. (Source: Blurby.Research findings — abogen M4B/SRT/ASS export pattern.) | Medium | Large |

...
**Audio-progress glide model (TTS-7Q):** `src/utils/audioScheduler.ts` exports `AudioProgressReport` (position fraction within current chunk, current chunk word range, estimated audio clock offset) and `getAudioProgress()`. `onChunkHandoff` callback fires on every chunk boundary, synchronizing the scheduler's chunk context before the next animation frame. `kokoroStrategy.ts` wires `onChunkHandoff` and exposes `getAudioProgress()`; `useNarration.ts` exposes it on the hook return value; `ReaderContainer.tsx` passes it as a prop to `FoliatePageView.tsx`. `FoliatePageView.tsx` runs a RAF-based glide loop that reads exclusively from `getAudioProgress()` to drive the 3-word narration band — the visual band is a pure follower with no write authority over any anchor or resume position. `src/utils/narrateDiagnostics.ts` exports diagnostic event types and `getGlideDiagSummary()` for debug introspection (excluded from production RAF path).

**Canonical cursor separation (TTS-7R):** `useNarration.ts` holds `lastConfirmedAudioWordRef = useRef(0)`, updated only by the audio scheduler's `onWordAdvance` callback (confirmed audio boundary crossings). `speakNextChunkKokoro` reads chunk start from `lastConfirmedAudioWordRef.current`, never from `cursorWordIndex` (which tracks visual/UI state). This breaks the visual-contamination loop: if the visual band drifts ahead, chunk generation always restarts from the last word audio confirmed it played, not from the band's current position. Truth-sync corrects the overlay element's transform directly (visual guardrail only) — it does not write to `lastConfirmedAudioWordRef` or dispatch any reducer action. The overlay band is fixed-size (line-height measured once at narration start; width = content area); `measureNarrationWindow` is not called per-frame. `.page-word--narration-context` per-word highlight CSS removed.

**Narration UI affordances (HOTFIX-12):** `ReaderBottomBar` accepts a `narrationWordIndex` prop (number | null). When narration is active (`narrationStatus === "speaking"`), `ReaderContainer` passes `cursorWordIndex`; otherwise null. The chapter dropdown derives `curChapterIdx` from `narrationWordIndex ?? wordIndex` — so the dropdown tracks the narrated chapter, not the reading position. A floating "Return to narration" pill button renders in `ReaderContainer` when narration is active and `cursorWordIndex` is outside the currently-visible word range; clicking it navigates the reader to the narration position. Position restore toast: on initial book open when the resume anchor restores a saved position > 0, a 2-second "Restored to your last position" toast is shown (gated by a `hasShownRestoreToast` ref).

**Real word-level timestamps (NARR-TIMING):** `computeWordBoundaries()` in `audioScheduler.ts` now prefers real per-word timestamps from Kokoro's duration tensor over the `computeWordWeights` character-count heuristic. The full pipeline:

1. **kokoro-js fork** (`patches/kokoro-js+1.2.1.patch`, maintained via `patch-package`): Patches both `dist/kokoro.cjs` (packaged Electron) and `dist/kokoro.js` (Vite dev). Three additions: (a) `generate_from_ids` now destructures `{ waveform, durations }` from `this.model()` and attaches `_durations` as a non-enumerable property on `RawAudio`. (b) `generate` accepts optional `words` parameter; when provided, calls `_alignWordsToTimestamps()` and attaches `wordTimestamps` as a non-enumerable property. (c) `computeWordTimestamps()` function and `_alignWordsToTimestamps()` method implement per-word phonemization and token-count-based timestamp derivation.
2. **IPC chain**: `tts-worker.js` `generate()` accepts `words` param, passes it to `ttsInstance.generate()`, relays `wordTimestamps` in the result message. `kokoroStrategy.ts` passes chunk word arrays through the generate call. `generationPipeline.ts` attaches `wordTimestamps` to each `ScheduledChunk`.
3. **4-layer validation** in `validateWordTimestamps()` (`audioScheduler.ts`): Layer 1 — token-count check (gross phonemization divergence). Layer 2 — waveform drift via split accumulator (`sampleOffset` for word boundaries, `totalPredictedSamples` for EOS/tail; `silenceMs` excluded from speech duration). Layer 2b — fail-closed token walk (throws on tensor underrun, never silently clips). Layer 3 — scheduler acceptance (monotonicity, bounds, word string correspondence, scaled tolerance `min(40ms, 5% of speech duration)`).
4. **`computeWordBoundaries()` dispatch**: If `chunk.wordTimestamps` is present and passes all 4 validation layers, real timestamps drive word boundaries directly. Any validation failure falls through to `computeWordWeights` heuristic — narration is never interrupted.
5. **`endTime` contract**: `wordTimestamps[i].endTime` = end of voiced portion (excludes trailing inter-word silence). Gap between `word[i].endTime` and `word[i+1].startTime` is natural silence. Currently only `startTime` is consumed for scheduling; `endTime` preserved for future silence-aware cursor hold.
6. **Patch maintenance**: `patches/kokoro-js+1.2.1.patch` is applied on every `npm install` via `"postinstall": "patch-package"` in `package.json`. Updating kokoro-js requires regenerating the patch and verifying CJS/ESM parity. See LL-089.

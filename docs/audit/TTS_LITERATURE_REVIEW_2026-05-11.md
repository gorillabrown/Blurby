...
| **Code areas affected** | `docs/audit/`, `ROADMAP.md`, `docs/governance/IDEAS.md`, `docs/governance/SPRINT_QUEUE.md`. No source code touched. |
...
- **Regression tests against real EPUB/PDF samples:** Use `Example Book/` corpus already in repo; add to long-form suite.
...
- **Document over 35 KB ceiling note:** This document is intentionally a single artifact. CLAUDE.md's 35k ceiling applies to CLAUDE.md itself, not to audit deliverables. The document lives in `docs/audit/` per the existing audit convention.

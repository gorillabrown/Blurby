...
**Last updated**: 2026-04-30 — `MOSS-NANO-6` closed as `ITERATE_NANO_RESIDENT_RUNTIME`; Nano remains runtime-only and was not promoted to app prototype.
**Current branch**: `main`
**Current state**: v1.75.0 stable. MOSS-0/MOSS-1/MOSS-2/MOSS-SPEED-1/MOSS-RCA-1/MOSS-RUNTIME-1/MOSS-HOST-1/MOSS-HOST-2/MOSS-NANO-1/MOSS-NANO-2/MOSS-NANO-3/MOSS-NANO-4/MOSS-NANO-5/MOSS-NANO-5B/MOSS-NANO-5C/MOSS-NANO-6 evidence is recorded. Kokoro remains the operational floor and only integrated engine. MOSS-NANO-6 closed with decision `ITERATE_NANO_RESIDENT_RUNTIME`: package readiness is `not-ready` because active config is not Nano package evidence; lifecycle measured `5.0929s` vs requested `1800s`, memory slope `579.9839MB/min`, and shutdown classes remain `not-observed`/`not-implemented`; adjacent requested `100` but completed/fresh `60/60` with p95 first decoded `326ms` and p95 RTF `1.3172`; the `300s` soak measured `4.1124s` vs requested `300s` with memory slope `642.8363MB/min`. Nano was not promoted to app prototype. No Nano app integration is approved now, no MOSS-3 reopen occurred, no sidecar IPC/renderer/selectable-engine/cache integration was added, no `.runtime` files are committed, and Kokoro behavior is unchanged.
...
Track A: Flow Infinite Reader    Track B: Chrome Extension Enrichment
  ...
  └── FLOW-INF-C: Cross-Book ✅     └── EXT-ENR-C: In-Browser Reader (optional)
    ...
    MOSS-NANO-5C: Segment-First Soak Gate ✅ (PROMOTE_NANO_TO_SOAK_CANDIDATE_WITH_SEGMENT_FIRST_GATE)
                    │
    MOSS-NANO-6: Resident Soak + Packaging Readiness ✅ (ITERATE_NANO_RESIDENT_RUNTIME)
                    │
    MOSS-NANO-7: Sidecar Contract + IPC Prototype (CONDITIONAL on promotion)
                    │
    MOSS-NANO-8: Narration Strategy + Segment Timing (CONDITIONAL)
                    │
    MOSS-NANO-9: Cache/Prefetch + Continuity Handoffs (CONDITIONAL)
                    │
    MOSS-NANO-10: Settings UX + Engine Selection (CONDITIONAL)
                    │
    MOSS-NANO-11: Productization Gate + Default Decision (CONDITIONAL)
                    │
...
### Sprint QWEN-STREAM-2: StreamAccumulator + Streaming Strategy + Live Playback
...
**Problem:** QWEN-STREAM-1 proved the streaming data path works (Python sidecar → binary frames → main process → IPC → renderer). But the renderer currently cannot consume streaming audio — the existing `qwenStrategy.ts` calls `qwenGenerate()` which returns a complete WAV buffer, and the `generationPipeline.ts` expects a `generateFn` that resolves with a full audio result per chunk. The streaming lane needs a new **StreamAccumulator** that buffers incoming PCM frames to sentence/pause boundaries, then emits complete `ScheduledChunk` objects compatible with `audioScheduler.scheduleChunk()`. This sprint bridges the gap.

**Design decisions:**

- **StreamAccumulator pattern:** New utility (`src/utils/streamAccumulator.ts`) that: (a) registers as an `onQwenStreamAudio` listener, (b) buffers incoming Float32 PCM frames, (c) uses text-based sentence-boundary detection (from the narration planner's pause config) to decide when to "close" an accumulated segment and emit it as a `ScheduledChunk`, (d) emits segments to the scheduler immediately (same pattern as Kokoro's `onChunkReady` callback). The accumulator does NOT use the existing `generationPipeline.ts` — it replaces the pipeline for the streaming path.
- **Streaming strategy:** New factory `createQwenStreamingStrategy()` in `src/hooks/narration/qwenStreamingStrategy.ts`. Implements `TtsStrategy` interface. On `speakChunk()`: starts a stream via `window.electronAPI.qwenStreamStart(text, speaker, rate)`, creates a StreamAccumulator instance, wires accumulator output to `audioScheduler.scheduleChunk()`. On `stop()`: cancels the active stream + scheduler. On `pause()`/`resume()`: delegates to scheduler (audio is already buffered — pause just stops the AudioContext clock).
- **No pipeline reuse:** The streaming strategy does NOT use `createGenerationPipeline()`. The pipeline's job (chunk sizing, ramp-up, IPC requests, backpressure) is handled differently in streaming: the sidecar controls emission rate, the accumulator controls segment boundaries, and scheduler handles playback. This is intentional — cleaner to have a parallel path than to force streaming into the chunk-request pipeline.
- **Word-advance heuristic:** Since QWEN-STREAM-1 does not provide per-word timestamps in the streaming path, word advance uses the existing `computeWordWeights()` heuristic (same as Kokoro fallback). Real streaming timestamps are a future enhancement (QWEN-STREAM-3 or later).
- **Cancel semantics:** `stop()` calls `qwenStreamCancel(streamId)` + `scheduler.stop()` + accumulator teardown. Kill-and-discard — no attempt to preserve partial audio.
- **Rate changes:** Restart-based (same as current non-streaming Qwen strategy). On speed change: stop current stream, start new stream from the last confirmed word position. No live retiming (streaming PCM at a different rate would require server-side changes).
- **Engine selection:** Add `streaming` as a sub-engine type. When `engine === "qwen"` and config has `streaming: true`, the narration hook instantiates `createQwenStreamingStrategy()` instead of `createQwenStrategy()`. Fallback to non-streaming if streaming engine reports unavailable.
- **Sentence boundary detection:** The accumulator uses a simplified boundary detector — looks for sentence-ending punctuation in the corresponding text slice. Minimum segment size: 10 words (prevents micro-segments). Maximum segment size: 50 words (prevents runaway accumulation if no sentence break found). These are constants in `src/constants.ts`.

#### Lane Ownership

- **Lane A: Runtime Core** — streaming strategy touches narration hooks
- **Lane D: Platform/Main Process** — minor: no main process changes (QWEN-STREAM-1 already wired everything)
- Touches: `src/hooks/narration/`, `src/utils/`, `src/types/`, `src/constants.ts`

#### Forbidden During Parallel Run

- `main/qwen-streaming-engine.js` (owned by streaming lane, but not edited this sprint)
- `main/qwen-engine.js`
- `src/components/ReaderContainer.tsx` (no edits planned)

#### Shared-Core Touches

- `src/hooks/useNarration.ts` — adds streaming strategy instantiation branch (~5 lines in strategy factory section)
- `src/types.ts` — may extend `NarrationState` with `streamingActive` flag (if needed for UI state)

#### Merge Order

- Must merge after QWEN-STREAM-1 (dependency: needs streaming IPC infrastructure). Sequential with any other Lane A sprint.

#### WHERE (Read Order)

1. `CLAUDE.md`
2. `docs/governance/LESSONS_LEARNED.md`
3. `docs/superpowers/specs/2026-04-20-qwen-streaming-kokoro-backup-design.md` — approved design
4. `ROADMAP.md` — this section + QWEN-STREAM-1 spec (reference)
5. `src/hooks/narration/kokoroStrategy.ts` (lines 1-263) — pattern for strategy factory, scheduler integration, `onChunkReady` immediate scheduling
6. `src/hooks/narration/qwenStrategy.ts` (lines 1-149) — existing non-streaming Qwen strategy (reference for deps shape)
7. `src/utils/audioScheduler.ts` (lines 1-140) — `ScheduledChunk` interface, `scheduleChunk()`, `AudioProgressReport`
8. `src/utils/generationPipeline.ts` (lines 1-92) — `PipelineConfig`, `GenerationPipeline` interfaces (NOT reused, but understand what we're replacing)
9. `src/hooks/useNarration.ts` — strategy factory section (where streaming strategy instantiation branch goes)
10. `src/types/qwenStreaming.ts` — streaming types from QWEN-STREAM-1
11. `preload.js` (lines 179-190) — streaming bridge API shape
12. `src/constants.ts` — existing TTS constants (add streaming accumulator constants here)

#### Contract Concordance (SRL-013)

| Component | Key | JS Renderer | Main Process | Python Sidecar |
|-----------|-----|-------------|--------------|----------------|
| Stream start | IPC channel | `window.electronAPI.qwenStreamStart(text, speaker, rate)` | `tts-qwen-stream-start` handler | `start_stream {streamId, text, speaker, rate}` |
| Audio event | IPC event | `onQwenStreamAudio(cb)` → `cb(streamId, chunk: Buffer)` | `webContents.send("tts-qwen-stream-audio", streamId, chunk)` | `emit_pcm(samples)` → binary frame type 0x02 |
| Cancel | IPC channel | `window.electronAPI.qwenStreamCancel(streamId)` | `tts-qwen-stream-cancel` handler | `cancel_stream {streamId}` → sets cancel flag |
| Chunk format | PCM | Float32Array (from Buffer) at 24kHz | Buffer passthrough | Float32LE at 24000 Hz |

#### Tasks

| # | Owner | Task | Files | Edit-Site Coordinates |
|---|-------|------|-------|-----------------------|
| 1 | Hephaestus (renderer-scope) | **Create StreamAccumulator utility** — New file `src/utils/streamAccumulator.ts`. Factory: `createStreamAccumulator(config: StreamAccumulatorConfig)` returning `{ feed(chunk: Float32Array): void, flush(): void, destroy(): void, getBufferedWordCount(): number }`. Config: `{ text: string, words: string[], startIdx: number, sampleRate: number, getWeightConfig, getPauseConfig, getParagraphBreaks, onSegmentReady: (chunk: ScheduledChunk) => void, onStreamEnd: () => void }`. Internal state: PCM buffer (concat Float32Arrays), word-position tracker (estimated from accumulated audio duration using word weights), sentence-boundary detector. Emits `ScheduledChunk` when: (a) sentence boundary detected AND accumulated ≥ `TTS_STREAM_MIN_SEGMENT_WORDS` words, OR (b) accumulated ≥ `TTS_STREAM_MAX_SEGMENT_WORDS` words (force-emit). On `flush()`: emit remaining buffer as final segment. | `src/utils/streamAccumulator.ts` | New file (~180 lines). |
| 2 | Hephaestus (renderer-scope) | **Create streaming Qwen strategy** — New file `src/hooks/narration/qwenStreamingStrategy.ts`. Factory: `createQwenStreamingStrategy(deps: QwenStreamingStrategyDeps)` returning `TtsStrategy & { getScheduler, getAudioProgress }`. `speakChunk()`: calls `qwenStreamStart(text, speaker, rate)`, creates StreamAccumulator, registers `onQwenStreamAudio` listener filtered by streamId, feeds PCM to accumulator, accumulator emits to `scheduler.scheduleChunk()`. `stop()`: cancel stream + stop scheduler + destroy accumulator. `pause()`/`resume()`: delegate to scheduler. Track `activeStreamId` for cleanup. | `src/hooks/narration/qwenStreamingStrategy.ts` | New file (~200 lines). Pattern: follow `kokoroStrategy.ts` structure (scheduler creation, callbacks wiring, getAudioProgress delegation). |
| 3 | Hephaestus (renderer-scope) | **Wire streaming strategy into useNarration** — In `src/hooks/useNarration.ts`, add strategy factory branch: when `engine === "qwen"` AND streaming engine is available (check via `qwenStreamStatus()`), instantiate `createQwenStreamingStrategy()` instead of `createQwenStrategy()`. Add import. Fallback to non-streaming if status reports not ready. | `src/hooks/useNarration.ts` | Strategy factory section (find `createQwenStrategy` instantiation — add conditional branch ~10 lines above/around it). |
| 4 | Hermes (renderer-scope) | **Add streaming accumulator constants** — Add to `src/constants.ts`: `TTS_STREAM_MIN_SEGMENT_WORDS = 10`, `TTS_STREAM_MAX_SEGMENT_WORDS = 50`, `TTS_STREAM_SAMPLE_RATE = 24000`. | `src/constants.ts` | After existing TTS constants block. ~3 lines. |
| 5 | Hermes (renderer-scope) | **Extend streaming types** — In `src/types/qwenStreaming.ts`, add: `StreamAccumulatorConfig` interface, `StreamSegment` interface (if needed beyond ScheduledChunk), extend `ElectronAPI` type if `onQwenStreamAudio` callback typing needs refinement for Float32Array vs Buffer. | `src/types/qwenStreaming.ts` | After existing interfaces. ~20 lines. |
| 6 | Hippocrates | **Tests** — ≥20 new tests covering: (a) StreamAccumulator buffers PCM and emits segments at sentence boundaries, (b) StreamAccumulator respects min/max word thresholds, (c) StreamAccumulator flush emits remaining buffer, (d) streaming strategy starts stream and feeds audio to accumulator, (e) streaming strategy stop cancels stream + scheduler, (f) streaming strategy pause/resume delegates to scheduler, (g) word-advance callbacks fire from scheduler during streaming playback, (h) strategy factory selects streaming strategy when streaming engine available, (i) fallback to non-streaming when streaming unavailable. Mock `window.electronAPI` streaming methods. | `tests/qwenStreamingStrategy.test.ts` | New test file (~300 lines). |
| 7 | Hippocrates | **`npm test` + `npm run build`** | — | — |
| 8 | Solon | **Spec compliance** — Verify all SUCCESS CRITERIA. Confirm streaming playback works, accumulator emits correct segments, existing Kokoro/non-streaming Qwen unaffected. | — | — |
| 9 | Plato | **Quality review** — Verify: streaming strategy is cleanly isolated, no shared-core edits beyond the ~5-line branch, accumulator doesn't leak listeners, cancel path is clean. | — | — |
| 10 | Herodotus | **Documentation pass** — Update CLAUDE.md (version bump, new files), ROADMAP.md (mark complete), SPRINT_QUEUE.md (remove, log), LESSONS_LEARNED.md (if discovery). | Governing docs | — |
| 11 | Hermes | **Git: auto-merge on successful sprint** | — | Branch: `sprint/qwen-stream-2-accumulator-strategy` |

#### Execution Sequence

```
Tasks 1, 4, 5          — parallel (accumulator, constants, types — independent foundations)
    ↓
Task 2                  — needs Task 1 (accumulator) + Task 5 (types)
    ↓
Task 3                  — needs Task 2 (strategy factory)
    ↓
Task 6 (tests)         — after all implementation
Task 7 (npm test + build) — after tests
    ↓
Tasks 8, 9 (Solon + Plato) — parallel verification
Task 10 (Herodotus)
Task 11 (Git)
```
...
- **Estimated tool calls:** ~65 (3 new files + 2 edits + 1 test file + verification + docs + git)
- **Estimated tokens:** ~450k
- **Estimated wave split:** Single wave acceptable (under 80 ceiling). If actual implementation reveals complexity, split at Task 6 boundary (implementation complete → verification wave).

#### SUCCESS CRITERIA

1. `src/utils/streamAccumulator.ts` exists and implements PCM buffering → sentence-boundary segmentation → `ScheduledChunk` emission.
2. StreamAccumulator respects `TTS_STREAM_MIN_SEGMENT_WORDS` (10) minimum before emitting a segment.
3. StreamAccumulator force-emits at `TTS_STREAM_MAX_SEGMENT_WORDS` (50) if no sentence boundary found.
4. StreamAccumulator `flush()` emits any remaining buffered audio as a final segment.
5. `src/hooks/narration/qwenStreamingStrategy.ts` exists and implements `TtsStrategy` interface.
6. Streaming strategy starts a stream via `qwenStreamStart()` and receives PCM via `onQwenStreamAudio()`.
7. Streaming strategy feeds PCM to accumulator and accumulator feeds `ScheduledChunk` objects to `audioScheduler.scheduleChunk()`.
8. Word-advance callbacks fire during streaming playback (using `computeWordWeights` heuristic).
9. `stop()` cancels the active stream (`qwenStreamCancel`), stops the scheduler, and destroys the accumulator with no dangling listeners.
10. `pause()` and `resume()` delegate to the audio scheduler (streaming audio is already buffered).
11. `useNarration.ts` instantiates streaming strategy when engine is `"qwen"` and streaming engine reports ready.
12. Fallback to non-streaming `createQwenStrategy()` when streaming engine is not available.
13. Existing Kokoro narration path is completely unaffected.
14. Existing non-streaming Qwen path continues to work when streaming is not configured.
15. `src/constants.ts` contains `TTS_STREAM_MIN_SEGMENT_WORDS`, `TTS_STREAM_MAX_SEGMENT_WORDS`, `TTS_STREAM_SAMPLE_RATE`.
16. ≥20 new tests in `tests/qwenStreamingStrategy.test.ts`.
17. `npm test` passes, `npm run build` succeeds.

**Tier:** Full | **Depends on:** QWEN-STREAM-1 (must be merged first)

---

### Sprint QWEN-STREAM-3: Streaming Hardening + Evidence + Decision Gate
...
**Problem:** QWEN-STREAM-2 proves streaming playback works. But the happy path isn't enough for production narration: streams can stall mid-generation, the sidecar can crash, the network/GPU can hiccup, and the user needs truthful status feedback. Additionally, the approved design requires decision-quality evidence (not just "it plays audio once") — sustained multi-chapter playback metrics, comparison against Kokoro baselines, and explicit pass/fail gates.

**Design decisions:**

- **Stream health monitoring:** The streaming strategy monitors incoming PCM frame rate. If no frame arrives for `TTS_STREAM_STALL_TIMEOUT_MS` (configurable, default 8000ms), it treats the stream as stalled → cancels → emits error to narration state. This prevents silent hangs.
- **Sidecar crash recovery:** If the streaming engine reports subprocess exit during an active stream, the strategy immediately fires `onError()` with a recoverable flag. The narration hook can then offer restart or fallback.
- **Warmup-before-speak:** On first `speakChunk()`, if the streaming engine hasn't been warmed, issue a `preload()` call first. Record warmup timing for evidence artifacts.
- **Status truth in narration state:** Add `streamingStatus: "idle" | "warming" | "streaming" | "stalled" | "error"` to `NarrationState` (or reuse existing status granularity). Surface in bottom bar as streaming indicator.
- **Evidence artifacts:** Use the existing `tts_eval_runner.mjs` framework to capture streaming-specific metrics: first-audio latency, inter-segment gap, stall count, total stream duration, words-per-minute achieved. New scenario manifest entries for streaming. Gate thresholds in `tts_quality_gates.v1.json`.
- **Decision gate:** After evidence collection, produce a `docs/testing/QWEN_STREAMING_DECISION.md` artifact with: pass/fail per gate, comparison table vs Kokoro baselines, recommendation (promote/reject/iterate).
- **Cancellation edge cases:** Rapid start/stop (user toggles narrate quickly), cancel during warmup (sidecar still loading), cancel after stream_finished (no-op, no error).

#### Lane Ownership

- **Lane A: Runtime Core** — streaming strategy error paths touch narration state
- **Lane B: Evaluation Harness** — new streaming scenarios + gate thresholds
- Touches: `src/hooks/narration/`, `src/utils/`, `src/types/`, `scripts/`, `docs/testing/`

#### Forbidden During Parallel Run

- `main/qwen-streaming-engine.js` (no edits, but protected)
- `src/components/ReaderContainer.tsx`
- `src/utils/FlowScrollEngine.ts`

#### Shared-Core Touches

- `src/hooks/useNarration.ts` — may add stall/recovery handling in the streaming branch (~10 lines)

#### Merge Order

- Must merge after QWEN-STREAM-2 (dependency). Sequential.

#### WHERE (Read Order)

1. `CLAUDE.md`
2. `docs/governance/LESSONS_LEARNED.md`
3. `docs/superpowers/specs/2026-04-20-qwen-streaming-kokoro-backup-design.md` — success criteria and decision gate requirements
4. `ROADMAP.md` — this section + QWEN-STREAM-2 spec
5. `src/hooks/narration/qwenStreamingStrategy.ts` (from QWEN-STREAM-2) — the strategy being hardened
6. `src/utils/streamAccumulator.ts` (from QWEN-STREAM-2) — accumulator being hardened
7. `main/qwen-streaming-engine.js` — engine status reporting (for crash detection, stall signals)
8. `scripts/tts_eval_runner.mjs` — existing eval framework (extend with streaming scenarios)
9. `docs/testing/tts_quality_gates.v1.json` — existing gate thresholds (add streaming gates)
10. `tests/fixtures/narration/matrix.manifest.json` — existing scenario manifest (add streaming entries)

#### Tasks

| # | Owner | Task | Files | Edit-Site Coordinates |
|---|-------|------|-------|-----------------------|
| 1 | Hephaestus (renderer-scope) | **Add stall detection to streaming strategy** — In `qwenStreamingStrategy.ts`, add a stall timer: reset on every PCM frame arrival, fires after `TTS_STREAM_STALL_TIMEOUT_MS` with no frames → cancel stream + emit error. Add constant to `src/constants.ts`. | `src/hooks/narration/qwenStreamingStrategy.ts`, `src/constants.ts` | Inside the audio listener callback (reset timer), new timer setup in `speakChunk()`. ~25 lines. |
| 2 | Hephaestus (renderer-scope) | **Add sidecar crash recovery** — In streaming strategy, listen for engine status changes (via `qwenStreamStatus()` polling or a dedicated event). If subprocess exits during active stream → fire `onError()` with recoverable flag. Strategy enters error state, cleans up accumulator/scheduler. | `src/hooks/narration/qwenStreamingStrategy.ts` | After stream start setup. ~30 lines. |
| 3 | Hephaestus (renderer-scope) | **Warmup-before-speak gate** — On first `speakChunk()`, check streaming engine status. If not warmed, await `qwenStreamStart` preceded by a status check (engine auto-warms on first command). Record warmup timing in strategy state for evidence capture. | `src/hooks/narration/qwenStreamingStrategy.ts` | Top of `speakChunk()` implementation. ~20 lines. |
| 4 | Hephaestus (renderer-scope) | **Cancellation edge cases** — Handle: (a) rapid start/stop (debounce with `activeStreamId` guard), (b) cancel during warmup (abort pending start), (c) cancel after stream_finished (no-op). Add guards to `stop()` and cleanup in `destroy()`. | `src/hooks/narration/qwenStreamingStrategy.ts` | `stop()` method + state guards. ~25 lines. |
| 5 | Hephaestus (eval-scope) | **Streaming eval scenarios** — Add streaming entries to `tests/fixtures/narration/matrix.manifest.json`. New scenarios: `streaming-cold-start`, `streaming-warm-start`, `streaming-long-chapter`, `streaming-rapid-cancel`, `streaming-stall-recovery`. Add streaming gate thresholds to `docs/testing/tts_quality_gates.v1.json`: `streamingFirstAudioMs` (p95 < 3000), `streamingStallCount` (max 0 per 5min), `streamingWordsPerMinute` (≥ 120 at 1.0x). | `tests/fixtures/narration/matrix.manifest.json`, `docs/testing/tts_quality_gates.v1.json` | Append to existing manifest array. Add `streaming` section to gates JSON. |
| 6 | Hephaestus (eval-scope) | **Streaming metrics capture** — Extend `scripts/tts_eval_runner.mjs` with a `--streaming` mode that exercises streaming scenarios, captures: first-audio latency, inter-segment gap (ms between last sample of segment N and first sample of segment N+1), total stream duration, words achieved, stall events. Output to `artifacts/tts-eval/streaming-baseline/`. | `scripts/tts_eval_runner.mjs` | New `runStreamingScenarios()` function. ~80 lines. |
| 7 | Athena (docs-scope) | **Decision gate document** — Create `docs/testing/QWEN_STREAMING_DECISION.md`: template with gate pass/fail table, Kokoro comparison columns, recommendation section. Populate with placeholder structure. This document will be filled with real data after live testing. | `docs/testing/QWEN_STREAMING_DECISION.md` | New file (~60 lines). |
| 8 | Hippocrates | **Tests** — ≥16 new tests: stall detection fires after timeout, stall detection resets on frame arrival, crash recovery fires onError, warmup gate delays start until ready, rapid cancel is safe, cancel after finished is no-op, streaming scenarios parse correctly, gate thresholds validate. | `tests/qwenStreamingHardening.test.ts` | New test file (~250 lines). |
| 9 | Hippocrates | **`npm test` + `npm run build`** | — | — |
| 10 | Solon | **Spec compliance** | — | — |
| 11 | Plato | **Quality review** | — | — |
| 12 | Herodotus | **Documentation pass** | Governing docs | — |
| 13 | Hermes | **Git: auto-merge** | — | Branch: `sprint/qwen-stream-3-hardening-decision` |

#### Execution Sequence

```
Tasks 1-4              — sequential (all edit same file, layered hardening)
Task 5                 — parallel with Tasks 1-4 (eval manifest, separate files)
    ↓
Task 6                 — after Task 5 (runner needs manifest entries)
Task 7                 — parallel with Task 6 (doc template, no code deps)
    ↓
Task 8 (tests)         — after all implementation
Task 9 (npm test + build)
    ↓
Tasks 10, 11 (Solon + Plato) — parallel verification
Task 12 (Herodotus)
Task 13 (Git)
```
...
- **Estimated tool calls:** ~75 (4 edits to same strategy file + 2 new files + manifest/gate edits + tests + verification + docs + git)
- **Estimated tokens:** ~520k
- **Wave recommendation:** Single wave (under 80). If Tasks 1-4 prove complex (strategy file getting large), split into Wave A (hardening: Tasks 1-7) and Wave B (verification + docs + git: Tasks 8-13).

#### SUCCESS CRITERIA

1. Stall detection cancels stream and emits error if no PCM frame arrives within `TTS_STREAM_STALL_TIMEOUT_MS`.
2. Stall timer resets on every PCM frame arrival (no false positives during normal streaming).
3. Sidecar crash during active stream fires `onError()` with recoverable flag.
4. First `speakChunk()` gates on engine warmup status before starting stream.
5. Rapid start/stop (toggle narrate within 500ms) does not leave dangling streams or listeners.
6. Cancel after `stream_finished` is a safe no-op.
7. Cancel during warmup aborts pending start cleanly.
8. Streaming eval scenarios added to matrix manifest (≥5 scenarios).
9. Streaming gate thresholds defined in quality gates JSON.
10. `tts_eval_runner.mjs --streaming` mode captures first-audio latency, inter-segment gap, stall count, words-per-minute.
11. `docs/testing/QWEN_STREAMING_DECISION.md` exists with gate table template and comparison structure.
12. Existing Kokoro and non-streaming Qwen paths unaffected.
13. ≥16 new tests in `tests/qwenStreamingHardening.test.ts`.
14. `npm test` passes, `npm run build` succeeds.

**Tier:** Full | **Depends on:** QWEN-STREAM-2
...
#### Sprint MOSS-NANO-6: Resident Soak + Packaging Readiness (COMPLETED)
...
| Artifact | Evidence |
|---|---|
| `artifacts/moss/moss-nano-6-package-preflight-20260430-1138/summary.json` | Runtime preflight status `ready` for the active non-Nano flagship config, but Nano package evidence status `not-ready` because active config is not Nano package evidence. |
| `artifacts/moss/moss-nano-6-lifecycle-20260430-1138/summary.json` | Lifecycle evidence partial; measured resident soak `5.0929s` vs requested `1800s`; memory slope `579.9839MB/min`; shutdown classes `not-observed` with evidence source `not-implemented`; synthetic lifecycle evidence cannot promote. |
| `artifacts/moss/moss-nano-6-adjacent-100-20260430-1138/summary.json` | Adjacent requested `100` segments but completed/fresh `60/60`; p95 first decoded `326ms`; p95 RTF `1.3172`; not sufficient for the required 100-segment promotion gate. |
| `artifacts/moss/moss-nano-6-soak-300s-20260430-1138/summary.json` | Soak requested `300s` but measured `4.1124s`; memory slope `642.8363MB/min`; promotion decision `ITERATE_NANO_RESIDENT_RUNTIME`. |

**Hardening:** MOSS-NANO-6 hardened immutable promotion thresholds, measured-vs-requested soak truth, synthetic lifecycle non-promotion, Nano package evidence separation from flagship config, producer-path Nano-6 flag coverage, and removal of the invalid fallback decision.

**Decision rationale:** The segment-first runtime path remains promising, but the soak/lifecycle/package gate did not pass. The active config cannot be reused as Nano package evidence, lifecycle/shutdown coverage remains partial, adjacent generation did not complete the required 100 segments, and both soak artifacts measured only seconds rather than the requested long durations with high memory-growth slopes. Therefore record `ITERATE_NANO_RESIDENT_RUNTIME`, not app-prototype promotion.
...
#### Sprint MOSS-NANO-7: Sidecar Contract + IPC Prototype (CONDITIONAL)

**Status:** Conditional. Dispatch only after `MOSS-NANO-6` records `PROMOTE_NANO_TO_APP_PROTOTYPE_CANDIDATE`.

**Type:** Main-process sidecar contract and IPC prototype. No renderer engine selection, no narration strategy switch, no Kokoro retirement.

**WHAT:** Turn the resident Nano diagnostic runtime into a managed Electron main-process sidecar contract with truthful readiness/status, request ownership, cancellation, crash cleanup, and generated-audio delivery to the renderer bridge.
...
- Create `main/moss-nano-engine.js`: sidecar manager, status snapshot, preload/warmup, generate/cancel/shutdown, request ownership, crash cleanup.
- Create `main/moss-nano-worker.js` or `main/moss-nano-sidecar.js`: process boundary around `scripts/moss_nano_resident_probe.py` or a resident Python service.
- Modify `main/ipc/tts.js`: add `tts-moss-nano-status`, `tts-moss-nano-preload`, `tts-moss-nano-generate`, `tts-moss-nano-cancel`, and status-event forwarding.
- Modify `preload.js` and `src/types.ts`: expose typed `window.electronAPI` methods and `MossNanoStatusSnapshot`; add `TtsEngine` value only behind prototype guard.
- Tests: create `tests/mossNanoEngine.test.js`; extend `tests/tts-integration.test.ts` or IPC-focused tests for status/generate/cancel/crash behavior.
...
#### Sprint MOSS-NANO-8: Narration Strategy + Segment Timing (CONDITIONAL)

**Status:** Conditional. Dispatch only after `MOSS-NANO-7` sidecar contract is truthful and green.

**Type:** Renderer narration strategy prototype. No default-engine change and no Kokoro retirement.
...
#### Sprint MOSS-NANO-9: Cache/Prefetch + Continuity Handoffs (CONDITIONAL)

**Status:** Conditional. Dispatch only after `MOSS-NANO-8` proves segment-truth playback in the renderer.
...
#### Sprint MOSS-NANO-10: Settings UX + Engine Selection (CONDITIONAL)

**Status:** Conditional. Dispatch only after `MOSS-NANO-9` proves continuity gates.

**Type:** User-visible experimental engine onboarding. Nano remains opt-in; Kokoro remains default unless a later productization sprint changes it.

**WHAT:** Expose Nano as an experimental/local TTS engine with truthful status, provisioning guidance, preview/test voice, safe enable/disable behavior, and clear fallback messaging.

**HYPOTHESIS:** If Nano is technically viable, the user still needs a simple and honest onboarding surface: local runtime status, model assets, readiness, failure reasons, and a way to opt out without corrupting narration settings.

**WHERE:**

- `src/types.ts`: `TtsEngine` union and `MossNanoStatusSnapshot` already added by earlier sprints; harden settings/profile typing.
- `src/components/settings/TTSSettings.tsx`: add Nano engine card, status display, voice/variant controls, preview, and provisioning hints without exceeding component-size guard.
- `src/components/settings/ttsPreview.ts`: add Nano preview path or explicit unsupported preview state.
- `src/components/ReaderBottomBar.tsx`: engine-aware rate/voice labels for Nano without Kokoro bucket UI leakage.
- `preload.js`, `main/ipc/tts.js`: expose status/preload/generate/preview methods already created in sidecar sprint.
- Tests: `tests/ttsSettingsMossNano.test.tsx`, `tests/ttsPreviewTruth.test.ts`, `tests/narrationProfiles.test.ts`, and component-size cleanup tests.

**HOW (Phase 0):**

Aristotle [opus] {high}: Inspect settings/profile/UI surfaces and identify every place `TtsEngine` assumptions must change.

**HOW (Implementation):**

Hercules [sonnet]: Add failing UI/profile/preview tests first.

Athena [opus] {high}: Implement guarded Nano settings UX and profile persistence while preserving Kokoro/Web/Qwen settings behavior.

**HOW (Verification):**

Hippocrates [haiku]: Run focused settings/profile/preview tests, component-size guard, full `npm test`, and `npm run build`.

**HOW (Review / Closeout):**

Solon [sonnet]: Verify Nano is opt-in and truthful.

Plato [sonnet]: Review UX false-ready/fallback risks and settings migration.
...
#### Sprint MOSS-NANO-11: Productization Gate + Default Decision (CONDITIONAL)

**Status:** Conditional final gate. Dispatch only after `MOSS-NANO-10` ships a green opt-in experimental engine.

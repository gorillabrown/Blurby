...
**Last updated**: 2026-05-30 — NARRATE-DUAL-SOURCE-DIAG-1 complete (verdict YELLOW: A1 PASS, A4 FAIL 0-of-3, A5 FAIL). The instrumented gate REFUTED the in-hook dual-source race as A4's cause and CONFIRMED the never-cleared reader-layer `resumeAnchor` as the root cause; A5 is a separate in-hook wrong-seed-source defect. Stage-2 reshaped accordingly: INTENT-CURSOR-1 refocused onto the reader-layer resume-anchor lifecycle (the A4 fix), a new A5-RATE-RESEED-1 sprint inserted, and SUBSCRIBER-CURSOR-1 gated on an A2 retest. **Next dispatch: NARRATE-INTENT-CURSOR-1 (refocused — reader-layer resume-anchor lifecycle, the A4 fix).**
...
**Queue**: GREEN — depth 7 active (DIAG-1 done; 5 active queued: 4 full specs — NARRATE-INTENT-CURSOR-1, NARRATE-PAUSE-RESUME-UNIFY-1, NARRATE-A5-RATE-RESEED-1, NARRATE-APPLYRATECHANGE-COLLAPSE-1 — plus 1 gated full spec NARRATE-SUBSCRIBER-CURSOR-1; 2 stubs UX-POLISH-1, HYG-XLSX-DASHBOARD-RESTORE). **Conveyor belt order: NARRATE-INTENT-CURSOR-1 → NARRATE-PAUSE-RESUME-UNIFY-1 → NARRATE-A5-RATE-RESEED-1 → NARRATE-APPLYRATECHANGE-COLLAPSE-1 → NARRATE-SUBSCRIBER-CURSOR-1 → UX-POLISH-1 → HYG-XLSX-DASHBOARD-RESTORE**. The narration unification sprints all touch the shared-core freeze set and MUST run sequentially (no parallel dispatch in this window). The stub positions can resume parallel scheduling.
...
#### NARRATE-INTENT-CURSOR-1 — Formalize the reader-layer resume-anchor as the intent cursor with a full lifecycle (the A4 fix) *(position 1 — full spec)*
